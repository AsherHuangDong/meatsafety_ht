    ...
    >
      ...
      >
        ...
        <FormItem label="职称" prop="jobTitle">
          <Input v-model="formValidate.jobTitle" style="width:200px;"></Input>
        </FormItem>
  ...
  methods: {
    ...
    sure(name) {
      ...
      for (let i in this.formValidate) {
        if (this.formValidate[i]&&(i!='createTime')&&i!='updateTime') {
          if (i == "jobTitle") {
            form.append("job_title", this.formValidate[i]);
          } else if (i == "wechat") {
            form.append("weChat", this.formValidate[i]);
          } else {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          _this.$axios.post("/api/user/updateUser", form).then(res => {
